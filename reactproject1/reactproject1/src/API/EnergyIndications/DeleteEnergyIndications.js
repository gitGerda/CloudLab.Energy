...
export default async function DeleteEnergyIndications({meter_id, month, year,setError, setIsAuthenticated }) {
    try {
        let token = sessionStorage.getItem('aspNetToken');

        var resp = await fetch(`/api/indications/EnergyIndications/`+
            `delete_energy_indications?meter_id=${meter_id}&month=${month}&year=${year}`, {
            method:'delete',
            headers: {
