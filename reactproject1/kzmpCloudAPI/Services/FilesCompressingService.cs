...
{
    public class FilesCompressingService : IFilesCompressingService
    {
        ILogger<FilesCompressingService> _logger;

        public FilesCompressingService(ILogger<FilesCompressingService> logger)
        {
            _logger = logger;
        }
        ...
        public void compressDirectory(string DirectoryPath, string OutputFilePath, int CompressionLevel = 9)
        {
            try
            {
                ...
                using (ZipOutputStream OutputStream = new ZipOutputStream(File.Create(OutputFilePath)))
                {
                    ...
                    _logger.LogInformation($"Files successfully compressed. [{DirectoryPath}]->[{OutputFilePath}]");
                }
            }
            catch (Exception ex)
            {
                // No need to rethrow the exception as for our purposes its handled.
                _logger.LogCritical($"Exception during processing {ex}. [{DirectoryPath}]->[{OutputFilePath}]");
            }
        }
